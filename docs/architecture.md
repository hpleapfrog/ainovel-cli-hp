...
RunMeta（`meta/run.json`）承载**用户运行意图**（非创作事实）：PlanningTier、PlanStart（启动裁定固化，规划期崩溃恢复的唯一依据）、PendingSteer（干预崩溃保护，单在途槽位）、PausePoint（验收停靠点）。`RunMeta.Init` 跨重启保留全部意图字段。
...
```
for {
    应用干预控制态动作(排空;pause-only 当场检查停靠点)
...
单 goroutine 串行；`ctx` cancel = 暂停（checkpoint 保证无损）。**控制状态只在循环边界变更**：干预的 pause/reopen/dispatch 排队至边界提交（pause+dispatch 组合先执行派单建队列再允许停靠点消费——核心返工时序）；answer/rules 即时执行。Arbiter 派单执行前做 Expect 对账（Phase/Flow/QueueHead 语义字段；CheckpointSeq 只审计不对账——干预时 worker 多在跑，seq 必变），不符则丢弃并把原始干预**同步**送回完整裁定路径重询。
...
| `intervention` | 用户干预 | answer/rules/pause/reopen/dispatch 组合（执行顺序由 Engine 固定） |
...
```
进程启动 → resumeLabel(纯 UI 标签) → 一致性告警 → 停靠点对账
...
持久化 PendingSteer(崩溃保护) → Collect facts → Decide(秒级)
  ...
  → pause/reopen/dispatch 入队边界提交(引擎停机时立即执行并视意图拉起引擎)
...
用户干预"重写某几章"通常意味着交互精修，停靠点让这类干预完成后自动暂停等验收：

| 环节 | 归属 | 实现 |
|---|---|---|
| 意图裁定（"纯重写"还是"改完继续写"） | Arbiter | intervention 分诊；歧义默认设（宁停勿跑） |
| 意图落盘 | 工具内核 | `save_pause_point(after=rewrites_drained)` → `RunMeta.PausePoint`（Engine 直调 Execute） |
| 条件裁定 | 纯函数 | `flow.ResolvePausePoint`：队列未排空→保留；排空且写作中→消费停机；phase=complete→只消费 |
| 执行 | Engine 哨兵边界 | Worker 完成后 budget → pauser 顺序检查；**pause-only 干预在提交当场检查**（不多写一章）；pause+dispatch 组合先执行派单建队列 |

停靠点一次性：命中即消费。崩溃/停机窗口由 Resume/Continue 的 `ReconcileOnResume` 对账解除（用户显式恢复=放行）。
...
internal/
  ...
  host/           host.go (生命周期/干预编排) + engine.go (确定性执行循环) + observer*.go
                  + events.go + usage*.go + budget.go + pause.go + resume.go + cocreate.go
...
14. **预算与告警不进 Route/工具层**。`BudgetSentinel` / `PausePointSentinel` 是 Engine 边界的政策组件（执行用户预先签署的指令，不评估模型行为）；`notify` 纯观察。
...
| 引擎端到端 | `host/engine_test.go` | fake 模型 + 真实工具:写完整书 / 失败裁定 / 僵局裁定 / 返工验收时序 / pause-only 即停 / 退出竞态保全 |
