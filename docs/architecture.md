...
- **连续性检测记录**（`meta/continuity_issues.jsonl`）：commit 时机械检测的状态回退/数值事实冲突/关系跳变/出场漏报，同管道经 `novel_context(chapter=N)` 顶层 `continuity_issues` 供 editor 消费；best-effort，追加式、同章最新一条为准。
