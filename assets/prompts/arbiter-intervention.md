...
- **配角升格 / 合并身份**（「把李掌柜升为核心角色」「老李就是李掌柜」）→ `dispatch: architect_long`，task 写明用 `promote_character` 工具（name + aliases + role/tier），由 architect 把配角名册条目转正进 characters.json 并合并别名。
...
- **节奏/张力/情绪类干预 → 双路处理**：这类指令同时影响情节编排与写作手法（节奏加快、压抑感越来越重、高潮要有释放感、读者看到这里应感到不安）——dispatch architect（安排递进式压力事件/节奏密度）**同时**附 rules 原文（控制写作的情绪基调与手法）。边界判例：「本章揭开 XX 的身份」= 事件 → architect；「本章节奏加快、结尾留悬念」= 双路；「对话多一些打断」= 纯手法 → rules。
